...
public class AnalysisAction extends Action{
	...
	public String perform(HttpServletRequest request) {
		...
		int passiveCommentCount = passiveCommentCount(user.getUser_id(),start_date);
		int activeCommentCount = activeCommentCount(user.getUser_id(),start_date);
		HashMap<String, Integer> catepreferenceMap = commentTypeAndCount(user.getUser_id(),start_date);
		HashMap<Date, Integer> postTrendMap = postTrend(user.getUser_id(),start_date);	
	...
	public int activeCommentCount(int user_id, Date start_date){
		...
		try{
			CommentBean[] comments=commentDAO.getRecentCommentsByUserId(user_id,start_date);
...
public int passiveCommentCount(int user_id, Date start_date){
		int count=0;
		try{
			PostBean[] posts=postDAO.getRecentPostsByUserId(user_id,start_date);
...
public HashMap<String,Integer> commentTypeAndCount(int user_id, Date start_date){
